...
첫 번째 플레이어의 두 번째 작업 - 첫 번째 플레이어가 pull하여 변경된 파일을 읽게 된다. 자신이 잘 못 만들었다는 것을 깨닫게 되고 이 프로젝트에 기여한 것이 없다는 죄책감에 불필요한 수정을 하게 된다.

두 번째 플레이어의 두 번째 작업 - 첫 번째 플레이어가 자신이 최종파일을 수정했다는 연락을 받고 변경된 사항을 확인하게 된다. 첫 번째 플레이어의 수정이 불필요한 부분이라는 것을 알고 reset한 후 tag로 이 파일은 수정할 필요가 없다고 남기며 프로젝트를 마치게 된다.
